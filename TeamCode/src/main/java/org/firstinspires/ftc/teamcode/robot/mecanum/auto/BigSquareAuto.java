...
public class BigSquareAuto extends SimulatableMecanumOpMode {
    ...
    public void init() {
        ...
        followPath = new PurePursuitPath(robot,
                new Waypoint(START_POSITION, 4, Subroutines.ENABLE_INTAKE),

                // Grab the farthest block and move foundation into position
                new StopWaypoint(-FIELD_RADIUS + 4 + 4, 24, 4, -0.75 * Math.PI, 3),
                new Waypoint(-FIELD_RADIUS + 4 + 4, 48, 16),
                new Waypoint(0, 48, 16, Subroutines.STOP_INTAKE),
                DEPOSIT_LOCATION,

                new Waypoint(0, 48, 16, Subroutines.ENABLE_INTAKE),
